...
function EditBlog() {
  ...
  const updatePost = async () => {
    ...
    try {
      ...
      if (thumbnail) {
        ...
        await updateBlog({ ...blogs }); // Pass the current state of blogs
      }
    } catch (error) {
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      await updateBlog({ ...blogs, thumbnail: url }); // Update with new thumbnail URL
    } catch (error) {
  ...
  const updateBlog = async (updatedBlog) => {
    ...
    try {
      await updateDoc(blogRef, {
        ...
        content: updatedBlog.content, // Ensure content is updated
      ...
      fetchUpdatedData(); // Fetch updated data after successful update
      navigate("/dashboard");
    } catch (error) {
  ...
  const fetchUpdatedData = async () => {
    try {
      ...
      if (docSnap.exists()) {
        const updatedBlogData = docSnap.data();
        setBlogs(updatedBlogData);
        setCurrentThumbnail(updatedBlogData.thumbnail);
      } else {
        console.log("No such document!");
        toast.error("Blog not found after update");
        navigate("/"); // Redirect to homepage or handle as needed
      }
    } catch (error) {
      console.error("Error fetching updated document:", error.message);
      toast.error("Error fetching updated blog data");
      navigate("/"); // Redirect to homepage or handle as needed
    }
  };
